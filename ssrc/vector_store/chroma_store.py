...
import chromadb
from chromadb.utils import embeddin g_functions
...
class RuleVectorStore:
    ...
    def embed_rules(self, rules: List[Rule], force_refresh: bool = False) -> None:
        """
        Embed all rules into ChromaDB.
        Skips rules already present unless force_refresh=True.
        """
    ...
    def query(self, diff_chunk: str, top_k: int = 5) -> List[Tuple[str, float]]:
        """
        Retrieve the top_k most semantically relevant rule IDs for a diff chunk.

        Returns:
            List of (rule_id, distance) tuples, sorted by relevance (lowest distance first).
        """
